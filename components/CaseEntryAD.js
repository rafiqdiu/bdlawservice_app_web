...
const registerSchema = yup.object({  //register validation schema
  case_no: yup.string().matches(/^[0-9]+$/, "Must be only digits")
...
function CaseEntryAD() {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const navigation = useNavigation();
  const [modalVisible, setModalVisible] = useState(false);
  const [userId, setUser_id] = useState(false);
  const [MySuperStore, setMySuperStore] = useState(null);
...
const [is_AD_permission, setIsADPermission] = useState(null);
const serializeErrors = (errors) => {
  return Object.values(errors).join(" ");
};
const _fetchTypeData = async () => {
  try {
    let url = `${BASE_URL_SIDDIQUE_ADMIN}/public/api/adcasetype`;
...
const locale = 'en-GB';
    ...
    React.useCallback(() => {
      ...
      .then(lawyer_code => {
        {setTimeout(
          async () => {
            try {
              const value = await AsyncStorage.getItem('MySuperStore'+lawyer_code);
              if (value !== null) {
         ...
         setMySuperStore('MySuperStore'+lawyer_code);
      })
  ...
  const _removeData = async () => {
    try {
      await AsyncStorage.removeItem(MySuperStore)
    } catch (error) {
    }
  };
...
const duplicateCaseCheck = async (parram) => {
  ...
  const totalCasesSubmit = async data => {     //async register user
    let url= `${BASE_URL_SIDDIQUE_ADMIN}/public/api/saveadnewcase`; 
        ...
        <View style={{flexDirection:'row', marginTop:15}}>
            <View style={styles.LawyerInfoLeft}></View>
                <View style={styles.LawyerInfo}>
                    <Text style={[styles.LawyerInfoText,{marginTop:-10}]}>
                      General Code : {userId}
                    </Text>
              ...
              {({
                ...
                <View style={{alignItems: 'center', marginTop:10}}>
                  <View style={styles.webMobile}>
                    ...
                    <View style={styles.pickerWrapper}>
                      <Picker
                        ...
                        style={{
                          ...styles.picker,color:type_id?'#000':'#7c7c7c',
                        }}
                 ...
                 <View style={{flexDirection:'row'}}>
                    ...
                    <View><Text style={{marginVertical:10, fontSize:22 }}>/</Text></View>
              <View style={styles.FormGroup}>
                  <TextInput
                   placeholderTextColor="#7c7c7c"
                   style={styles.inputstyle22}
                ...
                <View style={styles.FormGroup}>
                    <View style={styles.pickerWrapper}>
                      <Picker
                        selectedValue={pr_id}
                        style={{
                          ...styles.picker,color:pr_id?'#000':'#7c7c7c',
                        }}
                        onValueChange={(itemValue, itemIndex) => {
                          setPrId(itemValue);
        ...
        {totalCases && totalCases.length > 0 && totalCases.sort((a, b) => a.currentTime - b.currentTime).reverse().map((item, key) => (
           <View  style={[styles.hddata]} key={key+1}>
                    ...
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                      ...
                      <Text style={styles.textTilecln}>:</Text>
                      <Text style={styles.textDescription}>{item.case_no}/{item.year}</Text>
                    </TouchableOpacity>  
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
