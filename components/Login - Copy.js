     ...
     if (this._isMounted) {
      ...
      .then(value => {
        if (value != null && state.isConnected == true) {
          axios.post(`${BASE_URL_SIDDIQUE}/public/api/appsAndWebLogin?username=${value}&password=${this.state.password}&apps_user_type=1&is_mobile=1`)
          .then((res) => {
...
_handleNotificationResponse = response => {
  ...
  onLogin() {
    ...
    axios.post(`${BASE_URL_SIDDIQUE}/public/api/appsAndWebLogin?username=${this.state.userCode}&password=${this.state.password}&apps_user_type=1&is_mobile=1`)
    .then(res => {
  ...
  _loadsetItem= async () => {
    try {
      ...
      const Adaccess = await AsyncStorage.setItem('Adaccess',this.state.Adaccess);
      if (value != null){
