  ...
  componentDidMount() {
    ...
    if (this._isMounted) {
      ...
      .then((resData) => {
        ...
        axios
        .get(
          `${BASE_URL_SIDDIQUE}/public/api/appUpdate_count?l_id=${value}`
        )
        .then((resData) => {
          this.setState({ updateMsgcount: resData.data }); 
        })
      })
...
viewUpdateMsg=()=>{
  axios
  .get(
    `${BASE_URL_SIDDIQUE}/public/api/appUpdateView_c?l_id=${this.state.l_id}&id=${this.state.notificationData.id}`
  )
  .then((resData) => {
    axios
        .get(
          `${BASE_URL_SIDDIQUE}/public/api/appUpdate_count?l_id=${this.state.l_id}`
        )
        .then((resData) => {
          this.setState({ updateMsgcount: resData.data }); 
        }) 
...
closeMenu   = () => {
  ...
  <PaperProvider>
      ...
      <TouchableOpacity
        style={[styles.buttonMessage11,{backgroundColor: "#FFFFBD",}]}
        onPress={() => this.props.navigation.navigate('SearchListButton')}
      >
      <Text style={styles.buttonTextMessage111}>Search List By Date </Text>
      </TouchableOpacity>
      <View style={styles.BottomM}></View>
      <TouchableOpacity
        style={[styles.buttonMessage11,{backgroundColor: "#FFF",}]}
        onPress={() => this.props.navigation.navigate('CaseEntryButton')}
      >
      <Text style={styles.buttonTextMessage111}>New Case Entry </Text>
      </TouchableOpacity>
      <View style={styles.BottomM}></View>
      <TouchableOpacity
        style={[styles.buttonMessage11,{backgroundColor: "#FFF",}]}
        onPress={() => this.props.navigation.navigate('EntryCaseList', {param: 1})}
      >
      <Text style={styles.buttonTextMessage1}>Entry Requisition Slip</Text><Text style={{fontWeight:'bold',  fontSize:25,  color:'#ff0000'}}><Text style={{color:'#000'}}>= </Text>{this.state.hdRequestData?.length + this.state.adRequestData?.length}</Text>
      </TouchableOpacity> 
      <View style={styles.BottomM}></View>
      <TouchableOpacity
        style={[styles.buttonMessage11,{backgroundColor: "#FFF",}]}
        onPress={() => this.props.navigation.navigate('TotalCaseNotification')}
      >
      <Text style={styles.buttonTextMessage1}>New Case Added</Text><Text style={{fontWeight:'bold',  fontSize:25,  color:'#ff0000'}}><Text style={{color:'#000'}}>= </Text>{this.state.totalCaseList.length > 0?this.state.totalCaseList.length:0}</Text>
      </TouchableOpacity>
      <View style={styles.BottomM}></View>

        <TouchableOpacity
       style={[styles.buttonMessage11,{backgroundColor: "#FFF",}]}
        onPress={() => this.props.navigation.navigate('TotalCaseButton')}
      >
      <Text style={styles.buttonTextMessage111}>Total Case List </Text>
      </TouchableOpacity>
      <View style={styles.BottomM}></View>
      <TouchableOpacity
       style={[styles.buttonMessage11,{backgroundColor: "#FFFFBD",}]}
        onPress={() => this.props.navigation.navigate('Notification', {param: 1})}
      >
      <Text style={styles.buttonTextMessage1}>Latest Message</Text><Text style={{fontWeight:'bold',  fontSize:25, color:'#ff0000'}}><Text style={{color:'#000'}}>= </Text>{this.state.LmsgCount}</Text>
      </TouchableOpacity>
      <View style={styles.BottomM}></View>
      <TouchableOpacity
        style={[styles.buttonMessage11,{backgroundColor: "#FFFFBD",}]}
        onPress={() => this.props.navigation.navigate('RunningCourtButton')}
      >
      <Text style={styles.buttonTextMessage111}>Court Running Item & Result</Text>
      </TouchableOpacity>
      </View>
      <View style={styles.BottomM}></View>
      </View>
      ...
      <TouchableOpacity
              style={styles.buttonMessageH}
              onPress={() => this.props.navigation.navigate( 'BillInformation', {param: 2})}
            >
       <Text style={styles.buttonTextMessage1}>Bill Information</Text><Text style={{fontWeight:'bold',  fontSize:25, color:'#ff0000'}}><Text style={{color:'#000'}}>= </Text>{this.state.paymentCount}</Text>
       </TouchableOpacity> 
      {this.state.l_id == 100 && (() => 
      {
        if (this.state.l_id){
