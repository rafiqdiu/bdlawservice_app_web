...
import {BASE_URL_BDLAW} from './BaseUrl';
...
export default class TotalCaseHighCourtDivision extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
    this.state = {
  ...
  GetTotalHDData() {
    ...
    axios
      .post(
        `${BASE_URL_BDLAW}/public/api/totalcaselisthd?search_code=${this.state.lawyerCode}`
      )
      .then((res) => {
        if (this._isMounted) {
  ...
  getTotalCaseDetailsTypeWise(case_type_id) {
    ...
    axios
      .post(
        `${BASE_URL_BDLAW}/public/api/totalcaselisthddetails?l_id=${this.state.lawyerCode}&case_type_id=${case_type_id}`
      )
      .then((res) => {
        if (this._isMounted) {
  ...
  getCasesummaryDetailslw() {
    ...
    axios
      .post(
        `${BASE_URL_BDLAW}/public/api/casesummarydetailslw?l_id=${this.state.lawyerCode}`
      )
      .then((res) => {
        if (this._isMounted) {
    ...
    return (
      ...
      <TopBar lawyer_id={this.state.lawyerCode} lawyer_name={this.state.lawyer_name} ></TopBar> 
        ...
        <View  style={{height:windowHeight-80, }} >
          <ScrollView style={{ marginBottom: 5, bottom:0,  paddingBottom:80}} ref={(ref) => { this.scrollListReftop = ref; }}>
    ...
    <TouchableOpacity style={styles.TotalCaseTypeWiseBox}>    
          ...
          {this.state.caseDetails && this.state.caseDetails.length > 0 && this.state.caseDetails.map((item, key)=> {
            return (
                <View key={key}>
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                      <Text style={key % 2 == 0?styles.topbodyTextone:styles.topbodyTextone1}>{item.case_no}{item.case_no_plus}/{item.case_year }</Text>
                      <Text style={key % 2 == 0?styles.topbodyText:styles.topbodyText1}>{item.pr==1 ? 'Petitioner' : ''} {item.pr==2 ? 'Respondent' : ''}</Text>
          ...
          {Object.entries(this.state.caseDetailsLawyerWise).map(([key, value]) => {
            return (
              <View key={key}>
                {value.map((item, keys)=> {
                  return (
                    <TouchableOpacity key={keys} style={{ flexDirection: "row", padding: 0 }}>
        ...
        {this.state.caseDetailsLawyerWise && this.state.caseDetailsLawyerWise.length > 0 && this.state.caseDetailsLawyerWise.map((item, key)=> {
          return (
              <View key={key}>
                  <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                    <Text style={key % 2 == 0?styles.bodyText:styles.bodyText1}>{item.case_no}{item.case_no_plus}/{item.case_year }</Text>
