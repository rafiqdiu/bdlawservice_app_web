...
export default class HighCourtDivision extends Component {
      ...
      .then((value) => {
        if (value !== null) {
          ...
          axios  // Check isBlockSearchList start this.state.isBlockSearchList
            .post(
              `${BASE_URL_SIDDIQUE}/public/api/getSearchListBlockStatus?id=1`
            )
            .then((res) => {
          ...
          {          
              ...
              axios
              .get(
                `${BASE_URL_SIDDIQUE}/public/api/getHDMaxDate`
              )
              .then((resData) => {
              ...
              axios
              .get(
                `${BASE_URL_SIDDIQUE}/public/api/getHDSeventhDate`
              )
              .then((minDate) => {
  ...
  GetHDData() {
    ...
    axios
      .post(
        `${BASE_URL_SIDDIQUE}/public/api/casetoday?lawyerCode=${this.state.lawyerCode}&searchDate=${this.state.value ? moment(this.state.value).format(displayFormat) : moment(this.state.maxDate).format(displayFormat)}`
      )
      .then((res) => {
        if (this._isMounted) {
      ...
      axios
      .post(
        `${BASE_URL_SIDDIQUE}/public/api/is_draft_copy?searchDate=${this.state.value ? moment(this.state.value).format(displayFormat) : moment(this.state.maxDate).format(displayFormat)}`
      )
      .then((res1) => {
        if (this._isMounted) {
      ...
      axios
      .post(
        `${BASE_URL_SIDDIQUE}/public/api/prev_date?searchDate=${this.state.value ? moment(this.state.value).format(displayFormat) : moment(this.state.maxDate).format(displayFormat)}`
      )
      .then((res2) => {
        if (this._isMounted) {
      ...
      axios
      .post(
        `${BASE_URL_SIDDIQUE}/public/api/prev_result_datetime?searchDate=${this.state.value ? moment(this.state.value).format(displayFormat) : moment(this.state.maxDate).format(displayFormat)}`
      )
      .then((res3) => {
        if (this._isMounted) {
      ...
      axios
      .post(
        `${BASE_URL_SIDDIQUE}/public/api/previous_result?lawyerCode=${this.state.lawyerCode}&searchDate=${this.state.value ? moment(this.state.value).format(displayFormat) : moment(this.state.maxDate).format(displayFormat)}`
      )
      .then((res4) => {
        if (this._isMounted) {
      ...
      <TopBar lawyer_id={this.state.lawyerCode} lawyer_name={this.state.lawyer_name} ></TopBar>
        <View style={{ flexDirection: "row", flexWrap: "wrap", marginLeft: 2 ,marginTop:5 }}>
           {/* <DateTimePicker
            ...
            mode="date"
            minimumDate={new Date(this.state.minYear, this.state.minMonth, this.state.minDate)}
            maximumDate={new Date(this.state.maxYear, this.state.maxMonth, this.state.maxD)}
          /> */}
             <DatePickerModal
                       locale='en-GB'
                        mode="single"
         ...
         //  value={this.state.maxDate ? moment(this.state.maxDate).format(displayFormat) : moment(value).format(displayFormat)}
              ...
              return (
                <View  style={[styles.hddata]} key={item.id}>
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                      ...
                      <Text style={styles.textDescription}>{ item.court_list?.court_name}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                    ...
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                      ...
                      <Text style={styles.textTilecln}>:</Text>
                      <Text style={[styles.textDescription,{width:280}]}>{item.case_type?.type_name }</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                    ...
                    <TouchableOpacity style={{ flexDirection: "row", padding: 0 }}>
                      ...
                      <Text style={styles.textTilecln}>:</Text>
                      <Text style={{flex: 1, flexWrap: 'wrap', fontSize: 11, paddingTop:2,}}> { item.parties_info?.vs} </Text>
                    </TouchableOpacity>  
                </View>
              )
            })
